<template>
    <div class="register">
      ...
      <form style="padding: 10px;">
        ...
        <input type="button" @click="Check_Phone_Register()" class="btnlog" id="btn-sub" value="注册">
...
<script type="text/javascript">
  ...
  export default {
    ...
    methods:{
      Check_Phone_Register:function () {
        Check_Phone_Register();
      },
      sendMsg:function () {
        checkTelNum();
      }
  ...
  function checkphone00() {
    var returnStatus = true;

    $.ajax({
      url: '/Register/IsValidPhone',
      type: 'post',
      data: {
        phone: that.phone,
      },
      ...
      fail: function () {
        returnStatus = false;
        console.log("post /register/IsValidPhone failed");
      }
    });
    return returnStatus;
  }

  function Check_Phone_Register() {
    if (!Check_Phone()) return;
    if (!Check_Pwd()) return;
    if (!Check_Code()) return;

    $.ajax({
      url: '/Register/RegClick',
      type: 'post',
      data: {
        phone: that.phone,
      ...
      success: function (data) {
        ...
        if (state == 4) {
          that.$layer.msg("该手机号已注册");
          return false;
        } else if (state == 6) {
          that.$layer.msg("验证码错误");
          return false;
        } else if (state == 5) {
          that.$layer.msg("接收验证码手机号与用户名不匹配");
          return false;
        }
        else if (state == 11) {
          //注册成功
          that.$layer.msg("注册成功");
          location.href = "http://m.lwpoor.cn/loginNew";
        }
      },
      fail: function () {
        that.$layer.msg("系统异常");
